...
SELECT 
 Sale_Date_Converted
FROM 
 Portfolio_Project.dbo.Nashville_Housing -- its now in YYYY-MM-DD


--Identifying the rows where property address column contin NULL values.
...
WHERE 
 row_num > 1 
ORDER BY 
 PropertyAddress --104 rows found having duplicate values
